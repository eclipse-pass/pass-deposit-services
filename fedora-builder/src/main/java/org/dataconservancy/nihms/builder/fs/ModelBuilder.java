...
abstract class ModelBuilder {
    ...
    private DepositMetadata.Person createDepositAuthor(String name, String email) {
    ...
    private DepositMetadata.Person createDepositPerson(Person personEntity, boolean isPi,
                                                       boolean isCopi) {
        DepositMetadata.Person person = new DepositMetadata.Person();
    ...
    private DepositMetadata.Journal createDepositJournal(Journal journalEntity,
                                                         HashMap<URI, PassEntity> entities) {
    ...
    DepositSubmission createDepositSubmission(
        ...
        Journal journalEntity = (Journal)entities.get(submissionEntity.getJournal());
        DepositMetadata.Journal journal = createDepositJournal(journalEntity, entities);
        metadata.setJournalMetadata(journal);

        // Data from the Deposit resources
        for (URI depositUri : submissionEntity.getDeposits()) {
            Deposit depositEntity = (Deposit)entities.get(depositUri);
            // Available data for which there is no place in the existing model:
            //      id, status (enum), assigned ID, access URL, requested?, user action required?
            Repository repositoryEntity = (Repository)entities.get(depositEntity.getRepository());
            // Available data for which there is no place in the existing model:
        ...
        for (URI grantUri : submissionEntity.getGrants()) {
            ...
            // Available data for which there is no place in the existing model:
            //      id, name, url, localID
            Policy primaryPolicy = (Policy)entities.get(primaryFunderEntity.getPolicy());
            // Available data for which there is no place in the existing model:
            ...
            Person piEntity = (Person)entities.get(grantEntity.getPi());
            persons.add(createDepositPerson(piEntity, true, false));
            for (URI copiUri : grantEntity.getCoPis()) {
                Person copiEntity = (Person)entities.get(copiUri);
        ...
        for (URI workflowUri : submissionEntity.getWorkflows()) {
            Workflow workflowEntity = (Workflow)entities.get(workflowUri);
            // Available data for which there is no place in the existing model:
            //      id, name, step, steps
        }
